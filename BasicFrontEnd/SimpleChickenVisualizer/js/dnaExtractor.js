...
//DNA Color String (int) to hex color strings

var currentColorValue,
    quotientColorValue;

function getHexColorFromDNAColor(dnaColorString) {
  var fullHexValue = "",
      originalColorValue = parseInt(dnaColorString);

  currentColorValue = originalColorValue;

  while (fullHexValue.length < 6) {
    fullHexValue = fullHexValue + getNextHexChar();
  }

  return '#' + fullHexValue.split("").reverse().join("");
}

function getNextHexChar() {
  quotientColorValue = Math.floor(currentColorValue/16);
  var remainder = currentColorValue % 16;
  currentColorValue = quotientColorValue;
